...
try:
    ...
    def _get_object_size(data, start, dummy0, dummy1):
        return len(data) - start


    def _raw_to_dict(data, dict_class, start, dummy0, dummy1):
        return dict_class(data[start:])


    sys.modules['bson._get_object_size'] = _get_object_size
    sys.modules['bson._raw_to_dict'] = _raw_to_dict
...
def test_mongodb_connection():
    ...
    # Sử dụng URI đúng để tránh lỗi xác thực
    uri = "mongodb+srv://kiwi:trang%402005@film-users.10h2w59.mongodb.net/?retryWrites=true&w=majority&appName=film-users"
    dbname = os.getenv('MONGO_DBNAME', 'film-users')# Tên database từ URI
